...
def _static_deps(category=ScopeCategory.IN_SCOPE, generator=None):
    if generator is None:
        generator = AnswerGenerator(FakeLLM(ndjson(GOOD)), CHUNKS)
    return build_deps(
        conn=None,
        classifier=SimpleNamespace(classify=lambda q: SimpleNamespace(category=category, response=None)),
        retriever=SimpleNamespace(search=lambda query, k: PACK_RESULTS),
        generator=generator,
    )


def _client(schema, category=ScopeCategory.IN_SCOPE, generator=None):
    app = FastAPI()
    app.include_router(guest_router)
    state = SimpleNamespace(pool=FakePool(schema), static_deps=_static_deps(category, generator))
